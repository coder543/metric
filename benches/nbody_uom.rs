...
#[inline(never)]
pub fn uom_nbody() {
    let uom_nbodies: [UOMNBody; 4] = [UOMNBody {
    ...
    let g_uom: Quantity<uom::si::ISQ<P3, N1, N2, Z0, Z0, Z0, Z0>, uom::si::SI<f64>, f64> = unsafe { ::std::mem::transmute(6.674e-11) };
    let acceleration_zero: Acceleration = Acceleration::new::<meter_per_second_squared>(0.0);
    let time_step: Time = Time::new::<second>(0.1);

    let mut bodies = uom_nbodies.to_vec();
    for _ in 0..10000 {
        ...
        for a in 0..bodies.len() {
            ...
            for b in 0..bodies.len() {
                ...
                let force = g_uom / ((dist * dist) / (Ma * Mb));
                let Fx = force * (Dx / Dy);
                let Fy = force * (Dy / Dx);
                let Ax = Fx / Ma;
                let Ay = Fy / Ma;
